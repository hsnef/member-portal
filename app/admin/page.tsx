  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Note: members table doesn't have a status column
        // For now, showing all members as "active"
        const activeMembers = totalMembers

        const { count: lifetimeMembers } = await supabase
          .from('members')
          .select('*', { count: 'exact', head: true })
          .eq('current_level', 'Lifetime')

        const { count: annualMembers } = await supabase
          .from('members')
          .select('*', { count: 'exact', head: true })
          .eq('current_level', 'Annual')

        const { count: communityMembers } = await supabase
          .from('members')
          .select('*', { count: 'exact', head: true })
          .eq('current_level', 'Community')
        ...
        // Note: payments table doesn't have a status column
        // All records in payments table are considered completed
        const { data: revenueData } = await supabase
          .from('payments')
          .select('amount')

        const totalRevenue = revenueData?.reduce((sum, p) => sum + ((p as { amount: number }).amount || 0), 0) || 0
        ...
        // Fetch pending requests (status = 'Sent' means sent but not paid)
