...
export default function EditPurohitPage() {
  const router = useRouter()
  const params = useParams()
  const purohitId = params.id as string
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Purohit updated successfully!')
      router.push('/admin/purohits')
    } catch (error: any) {
      console.error('Error updating purohit:', error)
      alert(`Failed to update purohit: ${error.message}`)
    } finally {
      setSaving(false)
    }
  }
      ...
      <ProtectedRoute requiredRoles={['Office Staff', 'Office Manager', 'Admin']}>
        <AdminLayout>
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-600">Loading purohit...</p>
          </div>
        </AdminLayout>
      </ProtectedRoute>
    )
      ...
      <AdminLayout>
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Edit Purohit</h1>
